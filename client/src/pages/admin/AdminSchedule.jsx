...
export default function AdminSchedule() {
  ...
  const staffChipBase =
    "rounded-full px-3 py-1.5 text-xs font-semibold border transition shadow-sm";
  ...
  const selectedCount = useMemo(
    () => staffList.filter((s) => !!selected[s._id]).length,
    [staffList, selected]
  );
  ...
  return (
    <AdminLayout>
      <PageHeader
        title="Schedule"
        subtitle="Daily timeline by staff. Use filtered mode when coming from an appointment request."
      ...
      <Card className="mt-5 rounded-3xl border border-black/5">
        <CardBody className="p-5">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className={modeToggleWrap}>
                ...
                <button
                  ...
                  title={
                    !hasFilteredContext
                      ? "Open schedule from an appointment request to enable this"
                      : "Show only request staff"
                  }
                >
                  From request
                </button>
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white px-3 py-1.5">
                <CalendarDays className="h-4 w-4 text-black/50" />
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-2 rounded-full bg-cream-100 px-3 py-1.5 text-xs font-semibold text-black/70 ring-1 ring-black/5">
                <Users className="h-4 w-4 text-black/50" />
                Selected: <span className="text-hlblack">{selectedCount}</span> /{" "}
                <span className="text-hlblack">{staffList.length}</span>
              </span>

              <button className={smallBtn} onClick={selectAll} title="Select all staff">
                <span className="inline-flex items-center gap-2">
                  ...
                  {visibleStaff.map((s, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="px-4 py-4">
                          <div
                            className="relative w-full rounded-3xl border border-black/5 bg-white"
                          ...
                          >
                            ...
                            {wh && (
                              <div
                                className="absolute top-2 bottom-2 rounded-2xl bg-emerald-50"
                                style={{
                            ...
                            {(s.appointments || []).map((a) => {
                              ...
                              return (
                                ...
                                >
                                  <div
                                    className={[
                                      "group relative h-full w-full rounded-3xl border shadow-sm transition",
                                  ...
                                  >
                                    ...
                                    <div className="pointer-events-none absolute left-1/2 top-0 hidden -translate-x-1/2 -translate-y-[112%] group-hover:block">
                                      <div className="pointer-events-auto w-[280px] rounded-3xl border border-black/10 bg-white p-3 text-xs text-black shadow-2xl">
